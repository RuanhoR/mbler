...
export const utils = new class {
  ...
  DateToblock({
    ...
    return world.getDimension(this.dimensionList.includes(dimension) ? dimension: this.dimensionList[0]).getBlock(Location)
  }
  playerToData(player) {
  ...
  typeVerify(Object, ObjectSet) {
    const result = [];
    for (let [key, data] in Object.entries(Object)) {
