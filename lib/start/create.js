...
const npmStart = "npm:"
let dirname;
module.exports = async (workDir, _dirname, param = "") => {
  param = param.trim()
  dirname = _dirname;
  // 如果已经初始化了
  try {
    ...
    if (await utils.FileExsit(cacheDir)) {
      ...
      if (regex.test(param)) {
        ...
        await Load(cacheDir)
      } else if (await utils.FileExsit(utils.join(dirname, param))) {
          // 情况 : 输入路径
...
async function Load(cacheDir) {
  ...
  require(path.join(cacheDir, utils.join(dirname, main))).main(query)
}
...
async function copy(source, out) {
  await Promise.all((await fs.readdir(source))
