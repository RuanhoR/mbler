...
const findMatch = (version) => {
  ...
  for (const item of VERSION_TABLE) {
    ...
    if (compareVer(v, start) >= 0 && compareVer(v, end) <= 0) return {
      item,
      match: 'exact',
      start,
      end
  ...
  for (const item of sorted)
    if (compareVer(v, item.range[0]) >= 0) return {
      item,
      match: 'fallback',
      start: item.range[0],
      end: item.range[1]
...
module.exports.mcVersionGeter = {
  /**
   * @param {string} version Minecraft版本号
   * @param {boolean} [useBeta=false] 是否返回Beta版本
   */
