...
export default class Build extends BaseBuild {
  ...
  build(): Promise < void > {
    return this.start().catch(err => {
      ...
      LogNext(lang.build.error_message_log + errorMsg);
    });
  }
  ...
  async start(): Promise < void > {
    ...
    console.log(lang.build.build_info_header)
    console.table({
    ...
    // 最终完成
    console.log(`${lang.build.build_success} ${(Date.now() - startTime) / 1000}`);
  }
    ...
    for (let [id, title] of Object.entries(data.subpack)) {
      try {
        ...
        if (!(await utils.FileExsit(subpackPath))) {
          LogNext(lang.build.subpack_folder_not_found.replace('{id}', id));
          continue;
        }
        ...
        LogNext(lang.build.error_processing_subpack.replace('{id}', id).replace('{error}', (err as Error).stack));
      }
    }
  }

  async compileTypeScriptUnified(): Promise < void > {
    const tempMod = new Temp(path.join(os.tmpdir(), "mbler"));
    await tempMod.init();
    const tempDir = tempMod.dir;
    try {
      const rootFiles = await this.getAllTsFiles(this.outdir!);
      if (!rootFiles || rootFiles.length === 0) {
        LogNext(lang.build.ts_no_files);
        return;
      }
      let tsConfigJson = {
        compilerOptions: {
      ...
      if (await utils.FileExsit(path.join(this.baseCwd, "tsconfig.json"))) tsConfigJson = JSON.parse(await fs.readFile(path.join(this.baseCwd, "tsconfig.json"), "utf-8"))
      ...
      allDiagnostics.forEach(d => {
        ...
        if (d.file) {
          ...
          LogNext(lang.build.ts_diagnostics.replace('{file}', d.file.fileName).replace('{line}', (line + 1).toString()).replace('{character}', (character + 1).toString()).replace('{message}', msg));
        } else {
          LogNext(msg);
        }
      });
      ...
      LogNext(lang.build.ts_compilation_error + " 完成");
    } catch (err: any) {
      LogNext(lang.build.ts_compilation_error + err.stack);
      throw err;
    } finally {
  ...
  async processMinification(data: BuildData): Promise < void > {
    try {
      if (!Array.isArray(includes)) {
        LogNext(lang.build.includes_not_array);
        return;
      }
      await minify(
        this.outdir, {
      ...
      LogNext(lang.build.minification_error + err);
    }
  }
}