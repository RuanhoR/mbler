...
type Context = Record<string, t.Expression | { status: "wait" }>;
type MemberItem =
  | t.NewExpression
  | t.CallExpression
  | t.Super
...
class CompileJS {
  ...
  tre(node: t.Block) {
    ...
    for (let index in node.body) {
      ...
      if (item.type == "ImportDeclaration") {
        ...
        this.tre(t.blockStatement(item.block.body));
      } else if (item.type == "IfStatement") {
        const If = item.test;
        this.conditionalInTempImport(If, currenyContext);
        const nodes: t.Statement[] = [item.consequent];
        if (item.alternate) nodes.push(item.alternate);
        // if ... else ... make one by one
        this.tre(t.blockStatement(nodes));
        ...
        this.tre(t.blockStatement([item.body]));
        this.conditionalInTempImport(item.test, currenyContext);
  ...
  run() {
    if (!t.isBlock(this.node)) throw new Error("[compile error]: can't for a not block")
    this.tre(this.node);
  }
}
