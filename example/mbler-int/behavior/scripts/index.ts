...
class Lore {
  ...
  parseArr(): string[] {
    return [this.name || '', this.count.toString(16)]
  }
}
class baseUi {
  game: GameLib
...
export class utils {
  ...
  static removeItems(
    player: Player,
    { name, count }: RemoveItemsOptions = { name: '', count: 0 }
  ): void {
    const inventory = player.getComponent('inventory' as any)
      ?.container as Container
    if (!inventory) return
    ...
    for (let i = 0; i < inventory.size; i++) {
      ...
      if (RemoveNum >= num) {
        ...
        const remNum =
          RemoveNum >= 64
            ? 64
            : Math.max(RemoveNum, item.amount) -
              Math.min(RemoveNum, item.amount)
        item.amount = remNum
        RemoveNum -= remNum
