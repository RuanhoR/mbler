...
export class GameEventManager {
  ...
  private entityWrapperMap: Record<
    string,
    (logger: Logger, entity: any) => EntityWrapper
  >
  ...
  constructor(logger: Logger) {
    ...
    this.entityWrapperMap = {
      entity: (logger: Logger, entity: any) =>
        new EntityWrapper(logger, entity),
    }
  ...
  private _bindEvent(
    ...
    try {
      if (!this._isInternalCall) {
        this.logger.e(
          '错误：不允许直接调用 _bindEvent，请使用封装方法（如 PlayerAdd）'
        )
